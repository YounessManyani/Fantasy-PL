...
export class PlayerLoaderService implements IPlayerLoaderService {
  ...
  private calculateScore(position: Position, stats: PlayerStats): number {
    const ppgZ = zScore(
      stats.pointsPerGame,
      Z_MEANS.pointsPerGame,
      Z_STDDEVS.pointsPerGame
    );
    const goalsZ = zScore(
      stats.goalsScored,
      Z_MEANS.goalsScored,
      Z_STDDEVS.goalsScored
    );
    const assistsZ = zScore(stats.assists, Z_MEANS.assists, Z_STDDEVS.assists);
    const csZ = zScore(
      stats.cleanSheets,
      Z_MEANS.cleanSheets,
      Z_STDDEVS.cleanSheets
    );
    const savesZ = zScore(stats.saves, Z_MEANS.saves, Z_STDDEVS.saves);
    const gcZ = zScore(
      stats.goalsConceded,
      Z_MEANS.goalsConceded,
      Z_STDDEVS.goalsConceded
    );
    const bonusZ = zScore(stats.bonus, Z_MEANS.bonus, Z_STDDEVS.bonus);
    const yellowZ = zScore(
      stats.yellowCards,
      Z_MEANS.yellowCards,
      Z_STDDEVS.yellowCards
    );
    const redZ = zScore(stats.redCards, Z_MEANS.redCards, Z_STDDEVS.redCards);
    const xgiZ = zScore(
      stats.expectedGoalInvolvements,
      Z_MEANS.expectedGoalInvolvements,
      Z_STDDEVS.expectedGoalInvolvements
    );
