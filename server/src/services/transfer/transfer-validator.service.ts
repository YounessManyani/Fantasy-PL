...
export class TransferValidatorService implements ITransferValidatorService {
  ...
  validateTransfer(team: Player[], transfer: Transfer): ValidationResult {
    ...
    const hasPlayerOut = team.some(
      (p) => p.playerName === transfer.out.playerName
    );
    if (!hasPlayerOut) {
    ...
    const hasPlayerIn = team.some(
      (p) => p.playerName === transfer.in.playerName
    );
    if (hasPlayerIn) {
