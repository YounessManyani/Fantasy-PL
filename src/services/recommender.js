...
export class Recommender {
  constructor(store, fixtureService) {
    this.store = store;
    this.finder = new TransferFinder(store, fixtureService);
  ...
  recommend(team, options = {}) {
    ...
    if (!validation.valid) {
      ...
      return {
        ...
        transfers: [],
        explanation:
          "Team validation failed. Please fix the errors before requesting transfers.",
      };
    }

    // Find best transfers based on max allowed
    console.log(
      `[Recommender] Finding best transfers (max: ${maxTransfers})...`
    );
    let transfers = [];
  ...
  getOptions(team, bank = 0) {
    ...
    options.push({
      ...
      explanation:
        "Save your transfer for next gameweek to have 2 free transfers.",
    });
