...
import { POSITION_LIMITS, POSITION_NAMES } from '../../models/constants/positions.constants';
...
export class TeamValidatorService implements ITeamValidatorService {
  ...
  validate(players: Player[]): ValidationResult {
    ...
    if (players.length !== this.rules.startingXI) {
      errors.push(`Team has ${players.length} players (need exactly ${this.rules.startingXI})`);
    }
    ...
    for (const [club, count] of Object.entries(clubs)) {
      if (count > this.rules.maxPlayersPerClub) {
        errors.push(`${count} players from ${club} (max ${this.rules.maxPlayersPerClub})`);
      }
    }
    ...
    if (totalValue > this.rules.budget) {
      warnings.push(`Team value £${totalValue.toFixed(1)}m exceeds £${this.rules.budget}m budget`);
    }
