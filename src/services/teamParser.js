...
export class TeamParser {
  constructor(resolver, store, fixtureService) {
  ...
  async parse(teamText, options = {}) {
    ...
    console.log(`[TeamParser] Parsing with options:`, {
      strictMode,
      includeSuggestions,
      useLLM,
      gameweek,
    ...
    console.log(
      `[TeamParser] Found ${playerNames.length} player names to resolve`
    );
    ...
    for (const name of playerNames) {
      ...
      if (player) {
        ...
        if (seen.has(player.id)) {
          ...
          seen.add(player.id);
          results.players.push(this.formatPlayer(player, gameweek));
        }
      } else {
    ...
    console.log(
      `[TeamParser] Results: ${results.players.length} found, ${results.unknown.length} unknown`
    );
    if (useLLM) {
      console.log(
        `[TeamParser] LLM stats: ${results.llm_stats.resolved} resolved via LLM`
      );
    }
  ...
  formatPlayer(player, gameweek) {
    ...
    // Only enrich if we have a fixture service and a target GW
    if (this.fixtureService && gameweek) {
      const fx = this.fixtureService.getFixture(player.club_name, gameweek);
      const factor = fx ? this.fixtureService.getFactor(fx.fdr) : 1.0;

      return {
        ...base,
        next_fixture: fx || null, // keep explicit null if no data for consistency
        adjusted_score: Math.round(player.score * factor * 100) / 100,
      };
    }
