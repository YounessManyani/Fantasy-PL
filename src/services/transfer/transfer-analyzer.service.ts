import { Transfer, TransferImpact, TeamComparison, ComparisonStats, Player } from '../../types';
...
export interface ITransferAnalyzerService {
  ...
  applyTransfers(team: Player[], transfers: Transfer[]): { team: Player[]; changed: boolean };
...
export class TransferAnalyzerService implements ITransferAnalyzerService {
  ...
  calculateImpact(transfers: Transfer[]): TransferImpact {
    ...
    const totalCost = transfers.reduce((sum, t) => sum + Math.max(0, t.cost), 0);
  ...
  private formatTransfer(transfer: Transfer): string {
    ...
    if (transfer.meta?.gameweek) {
      ...
      const toText = (fx: any) =>
        fx
          ? `${fx.opponent} (${fx.home ? 'H' : 'A'}), FDR ${fx.fdr} — ${fx.difficulty}`
          : 'no fixture data';

      lines.push(
        `  GW${gw} fixtures → OUT: ${out.clubName}: ${toText(ofx)} | IN: ${inc.clubName}: ${toText(ifx)}`
      );

      if (transfer.meta.inAdjusted != null && transfer.meta.outAdjusted != null) {
        lines.push(
          `  Adjusted: IN ${transfer.meta.inAdjusted.toFixed(2)} vs OUT ${transfer.meta.outAdjusted.toFixed(2)} (uses FDR multipliers)`
        );
      }
    }
  ...
  private generateSummary(transfers: Transfer[]): string {
    ...
    if (transfers.length === 1) {
      return `Total impact: +${impact.scoreGain} expected score for £${Math.abs(impact.cost)}m`;
    }

    return `Total impact: +${impact.scoreGain} expected score across ${transfers.length} transfers for £${Math.abs(impact.cost)}m`;
  }
