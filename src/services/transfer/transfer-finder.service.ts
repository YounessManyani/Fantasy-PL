...
export interface ITransferFinderService {
  findSingleTransfer(team: Player[], budget: number, gameweek?: number): Transfer | null;
  findDoubleTransfer(team: Player[], budget: number, gameweek?: number): Transfer[];
  findAllSingleTransfers(team: Player[], budget: number, gameweek?: number): Transfer[];
}
...
export class TransferFinderService implements ITransferFinderService {
  ...
  findSingleTransfer(team: Player[], budget: number, gameweek?: number): Transfer | null {
    logger.debug('Finding single transfer', { teamSize: team.length, budget, gameweek });
  ...
  findDoubleTransfer(team: Player[], budget: number, gameweek?: number): Transfer[] {
    logger.debug('Finding double transfer', { teamSize: team.length, budget, gameweek });
  ...
  private getCandidates(
    ...
    const currentNames = new Set(
      team.map(p => p.playerName.toLowerCase())
    );
  ...
  private createTransfer(playerOut: Player, playerIn: Player, gameweek?: number): Transfer {
    ...
    const outFactor = outFixture ? this.fixtureCalculator.getFactor(outFixture.fdr) : 1.0;
    const inFactor = inFixture ? this.fixtureCalculator.getFactor(inFixture.fdr) : 1.0;
  ...
  private validateDoubleBudget(team: Player[], transfers: Transfer[], budget: number): boolean {
    const currentValue = team.reduce((sum, p) => sum + p.price, 0);
    const newValue = currentValue + transfers.reduce(
      (delta, t) => delta + (t.in.price - t.out.price),
      0
    );
