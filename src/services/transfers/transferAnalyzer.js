...
export class TransferAnalyzer {
    ...
    formatTransfer(transfer) {
      ...
      const mainLine = `• Replace ${out.name} (${out.position}, ${out.club}, £${out.price}m) ` +
                       `with ${inc.name} (${inc.position}, ${inc.club}, £${inc.price}m)`;
      
      const impactLine = `  → Score: +${transfer.gain.toFixed(2)} | ` +
    ...
    generateSummary(transfers) {
      ...
      if (transfers.length === 1) {
        return `Total impact: +${impact.score_gain} expected score for £${Math.abs(impact.cost)}m`;
      }
      
      return `Total impact: +${impact.score_gain} expected score across ${transfers.length} transfers for £${Math.abs(impact.cost)}m`;
    }
    ...
    compareTeams(oldTeam, newTeam) {
      ...
      return {
        ...
        new: newStats,
        improvements: {
          total_score: Math.round((newStats.total_score - oldStats.total_score) * 100) / 100,
          average_score: Math.round((newStats.average_score - oldStats.average_score) * 100) / 100,
          value_change: Math.round((newStats.total_value - oldStats.total_value) * 10) / 10
        }
      };
    }
